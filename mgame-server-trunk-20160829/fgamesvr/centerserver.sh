...
if [[ $CMD == 'start' ]] || [[ -z  $CMD ]]; then
    if [ -z "$3" ]; then
        CONFIG=$CUR_PATH/conf/config.center
    else
        CONFIG=$CUR_PATH/$3
    fi
    ...
    then
        sed -e 's/logger/--logger/' -e 's/daemon/--daemon/' $CONFIG > $CUR_PATH/conf/debug_config.center
        $CUR_PATH/../skynet/skynet $CUR_PATH/conf/debug_config.center
    fi
elif [ $CMD == 'stop' ]
then
    if [ ! -f $PID_FILE ] ;then
