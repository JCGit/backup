...
--[[
-- 添加活动的接口:
http://127.0.0.1:7602/gm?gmtype=addactivity&data={"id":260174,"name":"美味排行榜","desc":"巅峰排名活动美味排行榜","type":1008,"startTime":1465872468,"endTime":1466438399,"icon":"","state":2,"topConfig":{"top10":["10001:10,20001:20","10001:10,20001:20","10001:10,20001:20","10001:10,20001:20","10001:10,20001:20","10001:10,20001:20","10001:10,20001:20","10001:10,20001:20","10001:10,20001:20","10001:10,20001:20"],"wardTime":"23:59","mailTitle":"美味大冒险奖励","mailContent":"【巅峰排名来袭!恭喜您在《美味大冒险》中收集的星星数量达到排名第%d名，特发奖励一份，请注意查收，榜单上的额外奖励请通过QQ联系我们的客服雪儿2656247100】","inter":30}}
...
function CMD.__checkConfig(data)
    ...
    if not data.topConfig or type(data.topConfig) ~= "table" then
        logger.errorf("operate_activity_top_star.lua-> __checkConfig(...) not found rewards field or rewards field is not table")
        return false
    end
    ...
    if not string.match(data.topConfig.wardTime, '(%d+):(%d+)') then
        logger.debugf("operate_activity_top_star.lua-> __checkConfig(...) 时间格式错误 %s", topConfig.wardTime)
        return false
    end
...
function CMD.update()
    if not opend then
        skynet.call(".proxy", "lua", "star_open", activity_config)
        opend = true
    end
end
...
function CMD.activity_state_changed(state)
    ...
    if state == COMMON.OP_ACT_STATE.EXPIRED then
        skynet.call(".proxy", "lua", "star_close")
        opend = false 
    end
end
