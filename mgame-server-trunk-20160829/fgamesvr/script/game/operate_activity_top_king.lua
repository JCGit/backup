...
function CMD.__checkConfig(data)
    ...
    if not data.topConfig or type(data.topConfig) ~= "table" then
        logger.errorf("operate_activity_top_king.lua-> __checkConfig(...) not found topConfig field or topConfig field is not table")
        return false
    end
    ...
    if not string.match(data.topConfig.wardTime, '(%d+):(%d+)') then
        logger.debugf("operate_activity_top_king.lua-> __checkConfig(...) 时间格式错误 %s", topConfig.wardTime)
        return false
    end
...
function CMD.update()
    if not opend then
        skynet.call(".proxy", "lua", "king_open", activity_config)
        opend = true
    end
end
...
function CMD.activity_state_changed(state)
    ...
    if state == COMMON.OP_ACT_STATE.EXPIRED then
        skynet.call(".proxy", "lua", "king_close")
        opend = false 
    end
end
