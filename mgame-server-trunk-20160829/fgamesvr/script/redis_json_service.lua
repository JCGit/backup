...
local redis_balance = require "redis_balance"
...
function CMD.hdel_obj(table, id, flag)
    --[[
    local db
    if not redis_balance_tables[table] then
        db = id_db
    else
        db = redis_balance:get_work_node(id)
    end
    db:hdel(table, id)
    --]]
    id_db:hdel(table, id)
...
function CMD.hgetall_all_value(table)
    ...
    local lists = {}
    for i = 1, len do
        lists[tmp[i * 2 -1 ]] = json.decode(tmp[i * 2])
    end
    tmp = nil
    return lists;
end
...
skynet.start(function()
    ...
    for i,node in pairs(server_config.redis_conf) do
        ...
        --if db then
            --redis_balance_tables[node.id] = db
            --redis_balance:add_redis_node(node.name, db)
        --end
    end
    --redis_balance:rehash()
