...
-- value 代表pvp当天连胜的最大场数
]]

local rank_data = {}
local min_value = 0

local hash_table = 'king_rank'
local activity_cfg
-- 是否在活动期间内重启
local restart = false
local interval 
...
local function clear_data()
    logger.debugf("king_rank_service->clear_data at[%s]", utils.date_to_str(os.time()))
    ...
    if open then
        ...
        if restart then
            ...
            if next(rank_data) then
                ...
                logger.debugf("king_rank_service->flush() data is nil at[%s]", utils.date_to_str(now))
            end
            -- 定时器是按半小点整点刷新的，必然是小时的1/2或是整点 到零里06:00清除昨天的数据
            if date.hour == 6 and date.min == 0 then
                logger.debugf("king_rank_service->flush() clear yesterday data at[%s]", utils.date_to_str(os.time()))
                clear_data()
            end           
        end
        -- 去掉秒数，和game_proxy一致
        after = after * 60 - date.sec
        logger.debugf("king_rank_service->flush() now[%d] interval[%d] again at[%s]", now, after, utils.date_to_str(now + after))
    else
        logger.debugf("king_rank_service->flush() activity is not open at[%s]", utils.date_to_str(now))
        return
    end
...
local function send_mail_2center( uin, title, content, plugin )
    ...
    if not ok then
        logger.debugf("king_rank_service->send_mail_2center() failed uin[%d] title[%s] content[%s] plugin[%s]", uin, title, content, plugin)
    end
end
...
local function begin_award()
    ...
    for k,v in pairs(rank_data) do
        ...
        if award_cfg then
            ...
            skynet.send(log_service, "lua", "log_misc", COMMON.busilog_misc_type.misc_top_king, k, v.uin, v.name, v.value)
            logger.debugf("king_rank_service->begin_award() send mail success for uin[%d] plugin[%s]", v.uin, plugin)
        else
            logger.debugf("king_rank_service->begin_award award cfg not found for [%d]", k)
        end
    end
end
...
local function check_award()
    if open then
        ...
        logger.debugf("king_rank_service->check_award() now[%d] interval[%d] next_ticker[%s]", now, timer_tick, utils.date_to_str(now+timer_tick))
        ...
        logger.debug("king_rank_service->check_award activity is not open")
        return
    end
...
local function load_data()
    if redis_conn then
        ...
        logger.error("king_rank_service->load_data() redis conn is nil")
    end
end
...
local function update(uin, name, value)
    ...
    if len >= setting.top_num and min_value >= value then
        logger.debugf("king_rank_service->updat() value[%d] less than the min_value[%d]", value, min_value)
        return
    end
...
function CMD.open( status, config )
    if status and not open then
        ...
        check_award()
        flush()
        logger.debugf("king_rank_service->open() open at[%s]", utils.date_to_str(os.time()))
    elseif not status and open then 
        open = false
        clear_data()
        logger.debugf("king_rank_service->open() closed at[%s]", utils.date_to_str(os.time()))
    else
        logger.debugf("king_rank_service->open() what status[%s] cur status[%s]", tostring(status), tostring(open))
    end
...
--
function CMD.exit()
    logger.debug("king_rank_service exit")
    if open then
        if next(rank_data) then
            skynet.call(redis_conn, "lua", "hreset_obj", hash_table, utils.pack_hash_uin_key(rank_data))
        else
            logger.debugf("king_rank_service->exit() data is nil")
        end
    end
end
...
local function check_restart()
    ...
    if king_open then
        ...
        check_award()
        flush()
        logger.debugf("king_rank_service->check_restart() restart in open")
    end
end
...
skynet.start(function()
    ...
    skynet.dispatch("lua", function(_, src, command, ...)
        ...
        if not func then
            logger.warningf("king_rank_service unhandled message(%s)", command)
            return skynet.ret()
        end

        local ok, ret = xpcall(func, traceback, ...)
        if not ok then
            logger.warningf("king_rank_service handle message(%s) failed : %s src:%d", command, ret, src)
            return skynet.ret()
        end
        skynet.retpack(ret)
    end)
