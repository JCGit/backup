...
local function request(fd, method, host, url, recvheader, header, content)
	...
	if content then
		local data
		if method == "GET" then
			--GET /api/biz/service/verifySession.do?appId=2882303761517375342&session=11&uid=Denny&signature=....0...-...zG.8...S HTTP/1.1
			data = string.format("%s %s?%s HTTP/1.1\r\n%s\r\nAccept: */*", method, url, content, header_content)
			write(data)
			write(content)
		else
