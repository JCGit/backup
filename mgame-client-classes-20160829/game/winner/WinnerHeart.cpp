...
void WinnerHeart::onMenuItemAction(cocos2d::Ref* sender)
{
	...
	{
		WinnerSubActivity* winData = sgWinnerControl()->getWinnerSubID(WINNER_STAR + 1, pWidgt->getTag());
		...
		sgWinnerControl()->getWinnerProxy()->sendActGetRqst(WINNER_STAR + 1, pWidgt->getTag());

	}
}

bool WinnerHeart::init()
{
	bool ret = false;
	do
	{				
		mRoot = CSLoader::createNode("ui/WinnerHeart.csb");
		...
		mButton_4->addClickEventListener(CC_CALLBACK_1(WinnerHeart::onMenuItemAction, this));
		...
		for (int i = 0; i < 7; i++)
		{
			...
			mImageBtn[i]->addClickEventListener(CC_CALLBACK_1(WinnerHeart::onMenuItemAction, this));	

			cocostudio::Armature* pBtnJewel = cocostudio::Armature::create("fla_baoxiangDH");
			
			mImageBtn[i]->addChild(pBtnJewel,0,WINNER_STAR);		
			...
			pBtnJewel->getAnimation()->setMovementEventCallFunc(this, movementEvent_selector(WinnerHeart::jewelCallback));
...
void WinnerHeart::update(float delta)
{
	bool bfound = sgUserObj()->isHaveBadge(TitleType::BRAVE);
	mButton_4->setBright(bfound);

	WinnerActivityData* winData = sgWinnerControl()->getWinnerActList(WINNER_STAR+1);
	if (winData == NULL) return;
	...
	{				
		cocostudio::Armature* jewelArm = dynamic_cast<cocostudio::Armature*>(mImageBtn[i]->getChildByTag(WINNER_STAR));
...
void WinnerHeart::jewelCallback(cocostudio::Armature * pAr, cocostudio::MovementEventType type, const std::string& aniName)
{
