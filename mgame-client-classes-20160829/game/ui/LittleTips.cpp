...
bool LittleTips::init(EDT dType, bool isChange){
	...
	mTips = sgResManager()->getAramatur("fla_battle_xiaotishi");//cocostudio::Armature::create("fla_battle_xiaotishi");//小提示字样
...
bool LittleTips::init(EDT texType, std::string textStr, bool isPerson, Point panelPos, bool isChange){
	...
	if (isPerson)
	{
		auto person = ImageView::create("npc2small.png");//资源问题
