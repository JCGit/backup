...
int MicroPather::SolveForNearStates( void* startState, std::vector< StateCost >* nearvar, float maxCost )
{
	...
	*/
	...
	for( PathNode* pNode=closedSentinel.next; pNode != &closedSentinel; pNode=pNode->next ) {
		if ( pNode->totalCost <= maxCost ) {
			...
			nearvar->push_back( sc );
		}
	}
#ifdef DEBUG
	for( unsigned i=0; i<nearvar->size(); ++i ) {
		for( unsigned k=i+1; k<nearvar->size(); ++k ) {
			MPASSERT( nearvar->at(i).state != nearvar->at(k).state );
		}
	}
#endif
