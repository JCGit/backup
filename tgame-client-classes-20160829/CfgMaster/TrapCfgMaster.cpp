...
int CTrapCfgMaster::ValidateCfg()const
{
	int iRet = 0;
	...
	for(int iIdx = 0; iIdx < m_stCfg.trapcfg_size(); iIdx++)
	{
		unsigned int uiCfgID = m_stCfg.trapcfg(iIdx).id();
...
const TrapCfg* CTrapCfgMaster::GetTrapCfgByID(const unsigned int uiCfgID)
{
	...
	return &(m_stCfg.trapcfg(it->second));
}
