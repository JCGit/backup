...
int CFloorBuffCfgMaster::ValidateCfg()const
{
	int iRet = 0;
	...
	for(int iIdx = 0; iIdx < m_stCfg.floorbuffcfg_size(); iIdx++)
	{
		unsigned int uiCfgID = m_stCfg.floorbuffcfg(iIdx).id();
...
int CFloorBuffCfgMaster::SetCfg()
{
	int iRet = 0;
...
const FloorBuffCfg* CFloorBuffCfgMaster::GetFloorBuffCfgByID(const unsigned int uiCfgID)
{
	...
	return &(m_stCfg.floorbuffcfg(it->second));
}
