...
int CBuildItemCfgMaster::ValidateCfg()const
{
	int iRet = 0;
	...
	for(int iIdx = 0; iIdx < m_stCfg.builditemcfg_size(); iIdx++)
	{
		unsigned int uiCfgID = m_stCfg.builditemcfg(iIdx).itemid();
...
int CBuildItemCfgMaster::SetCfg()
{
	int iRet = 0;
...
BuildItemCfg* CBuildItemCfgMaster::GetBuildItemCfgByID(const unsigned int uiCfgID)
{
	...
	return m_stCfg.mutable_builditemcfg(it->second);
}