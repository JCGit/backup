...
int CConstantCfgMaster::ValidateCfg()const
{
	int iRet = 0;
	...
	for(int iIdx = 0; iIdx < m_stCfg.constantcfg_size(); iIdx++)
	{
		unsigned int uiCfgID = m_stCfg.constantcfg(iIdx).id();
...
const ConstantCfg* CConstantCfgMaster::GetConstantCfgByID(const unsigned int uiCfgID)
{
	...
	return &(m_stCfg.constantcfg(it->second));
}
