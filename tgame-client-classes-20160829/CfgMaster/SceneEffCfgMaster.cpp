...
int CSceneEffCfgMaster::ValidateCfg()const
{
	int iRet = 0;
	...
	for(int iIdx = 0; iIdx < m_stCfg.sceneeffcfg_size(); iIdx++)
	{
		unsigned int uiCfgID = m_stCfg.sceneeffcfg(iIdx).id();
...
const SceneEffCfg* CSceneEffCfgMaster::GetSceneEffCfgByID(const unsigned int uiCfgID)
{
	...
	return &(m_stCfg.sceneeffcfg(it->second));
}