...
int CMapCfgMaster::ValidateCfg()const
{
	int iRet = 0;
	...
	for(int iIdx = 0; iIdx < m_stCfg.mapcfg_size(); iIdx++)
	{
		unsigned int uiCfgID = m_stCfg.mapcfg(iIdx).id();
...
MapCfg* CMapCfgMaster::GetMapCfgByID(const unsigned int uiCfgID)
{
	...
	return m_stCfg.mutable_mapcfg(it->second);
}