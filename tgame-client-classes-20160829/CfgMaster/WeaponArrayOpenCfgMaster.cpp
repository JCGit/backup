...
int CWeaponArrayOpenCfgMaster::ValidateCfg()const
{
	int iRet = 0;
	...
	for(int iIdx = 0; iIdx < m_stCfg.cells_size(); iIdx++)
	{
		unsigned int uiCfgID = m_stCfg.cells(iIdx).idx();
...
int CWeaponArrayOpenCfgMaster::SetCfg()
{
	int iRet = 0;
...
const WeaponArrayOpenCfg* CWeaponArrayOpenCfgMaster::GetWeaponArrayOpenCfgByID(const unsigned int uiCfgID)
{
	...
	return &(m_stCfg.cells(it->second));
}
