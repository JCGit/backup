...
const FightGeekAwardCfg* FightGeekAwardCfgMaster::getFightGeekAwardActCfg(unsigned int uLevel)
{
	...
	return &(m_stCfg.fightgeekawardcfg(iter->second));
}

int FightGeekAwardCfgMaster::ValidateCfg()const
{
	int iRet = 0;

	set<unsigned int> stFightGeekAwardSet;

	for(int i = 0; i < m_stCfg.fightgeekawardcfg_size(); i++)
	{
		unsigned int uLevel = m_stCfg.fightgeekawardcfg(i).level();
...
int FightGeekAwardCfgMaster::SetCfg()
{
	int iRet = 0;
...
unsigned int FightGeekAwardCfgMaster::getFightGeekAwardTypeNum()
{
