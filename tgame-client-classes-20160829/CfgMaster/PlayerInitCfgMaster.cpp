...
int CPlayerInitCfgMaster::ValidateCfg()const
{
	int iRet = 0;
	...
	for(int iIdx = 0; iIdx < m_stCfg.playerinitcfg_size(); iIdx++)
	{
		unsigned int uiCfgID = m_stCfg.playerinitcfg(iIdx).playerid();
...
int CPlayerInitCfgMaster::SetCfg()
{
	int iRet = 0;
...
const PlayerInitCfg* CPlayerInitCfgMaster::GetPlayerCfgByID(const unsigned int uiCfgID)
{
	...
	return &(m_stCfg.playerinitcfg(it->second));
}
