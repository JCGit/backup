...
int CBulletCfgMaster::ValidateCfg()const
{
    int iRet = 0;
    ...
    for(int iIdx = 0; iIdx < m_stCfg.bulletcfg_size(); iIdx++)
    {
        unsigned int uiCfgID = m_stCfg.bulletcfg(iIdx).bulletid();
...
const BulletCfg* CBulletCfgMaster::GetBulletCfgByID(const unsigned int uiCfgID)
{
    ...
    return m_stCfg.mutable_bulletcfg(it->second);
}
