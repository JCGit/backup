...
int CWarAwardConditionCfgMaster::ValidateCfg()const
{
	int iRet = 0;
	...
	for(int iIdx = 0; iIdx < m_stCfg.warawardconditioncfg_size(); iIdx++)
	{
		unsigned int uiCfgID = m_stCfg.warawardconditioncfg(iIdx).type();
...
int CWarAwardConditionCfgMaster::SetCfg()
{
	int iRet = 0;
...
const WarAwardConditionCfg* CWarAwardConditionCfgMaster::GetWarAwardConditionCfgByID(const unsigned int uiCfgID)
{
	...
	return &(m_stCfg.warawardconditioncfg(it->second));
}
