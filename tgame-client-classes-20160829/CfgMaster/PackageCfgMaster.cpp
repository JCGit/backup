...
int CPackageCfgMaster::ValidateCfg()const
{
	int iRet = 0;
	...
	for(int iIdx = 0; iIdx < m_stCfg.packagecfg_size(); iIdx++)
	{
		unsigned int uiCfgID = m_stCfg.packagecfg(iIdx).packageid();
...
const PackageCfg* CPackageCfgMaster::GetPackageCfgByID(const unsigned int uiCfgID)
{
	...
	return &(m_stCfg.packagecfg(it->second));
}
