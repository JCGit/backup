...
ExRichElementText* ExRichElementText::create(int tag, const ccColor3B& color, GLubyte opacity, Label *pLabel) {
	ExRichElementText *pRet = new ExRichElementText();

	if(pRet != NULL && pRet->init(tag, color, opacity, pLabel)) {
		pRet->autorelease();
	} else {
		CC_SAFE_DELETE(pRet);
	}
...
bool ExRichElementText::init(int tag, const ccColor3B& color, GLubyte opacity, Label *pLabel) {
				...
				case EX_RICH_TEXT:
					{
						...
						Label *pLabel = pElmtText->_pLabel;
						pLabel->setText(s);

						std::vector<std::string> texts;
						CToolUtils::parseUTFStr(s, texts);
						std::vector<std::string> stVals;
						float calcWidth = _leftSpaceWidth;

						if(calcWidth - pLabel->getSize().width < 0) {
							std::string tmp = "";

							for(unsigned int i = 0; i < texts.size(); i++) {
								pLabel->setText(tmp + texts[i]);

								if(calcWidth - pLabel->getSize().width < 0) { // 超出宽度另起一行
						...
						for(unsigned int i = 0; i < stVals.size(); i++) { // stVals的size应该>=1
							if(i == 0) {
								pLabel->setText(stVals[i]);
								_leftSpaceWidth -= pLabel->getSize().width;
								pushToContainer(pLabel);
							} else {
								addNewLine();
								Label *pNewLabel = dynamic_cast<Label*>(pLabel->clone());
								pNewLabel->setText(stVals[i]);
								_leftSpaceWidth -= pNewLabel->getSize().width;
...
void ExRichText::startElement(void *ctx, const char *name, const char **atts) {
	...
	if(std::strcmp(name, "br") == 0) {
		...
		ui::UILabel *pLabel = ui::UILabel::create();
		pLabel->setName(name.c_str());
		pLabel->setFontName(face);
		pLabel->setFontSize(size);
		pLabel->setColor(color);

		if(touchable) {
			pLabel->setTouchEnabled(touchable);
			pLabel->addTouchEventListener(this, toucheventselector(ExRichText::touchEventHandler));
		}

		pElmt = ExRichElementText::create(0, color, 0xff, pLabel);
...
void ExRichText::touchEventHandler(CCObject *pSender, TouchEventType type) {
