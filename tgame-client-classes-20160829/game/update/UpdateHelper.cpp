...
int ZipFile::ExtractTo(const char * root) {
    char pathtmp[1024];
    char buf[16*1024];
    ...
    while (ret == UNZ_OK) {
        ...
        *m_piProgress = int(iCurDoFileNum*100/stGlobalFileInfo.number_entry); //进度
        ...
        folder = UpdateHelper::GetPath(pathtmp);
        ...
        fs = fopen(pathtmp, "wb");
        if (fs == NULL) {
            CCLOG("[error]ExtractTo, Fail to open file %s", pathtmp);
            retval = -1;
            goto HANDLE_FILE_ERROR;
        }
        while (true) {
        ...
        if (ret == 0) {
            CCLOG("[trace]ExtractTo, finish extract file %s", pathtmp);
#if (CC_TARGET_PLATFORM != CC_PLATFORM_WIN32) 
            //chmod(pathtmp, 755); //全加吧
            chmod(pathtmp, S_IRUSR | S_IWUSR | S_IXUSR | S_IRGRP | S_IXGRP | S_IROTH | S_IXOTH  );
#endif
        } else {
            retval = -1;
        }
