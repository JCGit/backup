...
CFightEndlessRankPanel::CFightEndlessRankPanel(){
	m_pTableView = NULL;
	m_pRankDemo = NULL;
	m_uCellCount = 0;
}
...
bool CFightEndlessRankPanel::init(){
	bool bRet = false;
...
void CFightEndlessRankPanel::initUI(){
	...
	m_pCloseBtn->addTouchEventListener(this, toucheventselector(CFightEndlessRankPanel::touchEventHandler));
	m_pEnterBtn->addTouchEventListener(this, toucheventselector(CFightEndlessRankPanel::touchEventHandler));
}

CCSize CFightEndlessRankPanel::cellSizeForTable(CCTableView *table){
	return CCSizeMake(table->getContentSize().width, 50);
}

unsigned int CFightEndlessRankPanel::numberOfCellsInTableView(CCTableView *table){
	return m_uCellCount;
}

CCTableViewCell* CFightEndlessRankPanel::tableCellAtIndex(CCTableView *table, unsigned int idx){
...
void CFightEndlessRankPanel::tableCellTouched(CCTableView* table, CCTableViewCell* cell){
...
void CFightEndlessRankPanel::updateUI(){
	const CSRankQueryRsp& stRankInfo = P_RANK_CTL->getRankInfo();
...
void CFightEndlessRankPanel::updateMyRank(int rank){
	if(rank < 0){
...
void CFightEndlessRankPanel::open(){
	BasePanel::open();
...
void CFightEndlessRankPanel::close(){
	BasePanel::close();
}

void CFightEndlessRankPanel::touchEventHandler(CCObject *pSender, TouchEventType type){
	...
	switch (type) {
		case TOUCH_EVENT_ENDED:
			if(strcmp(pszWidgetName, "close_btn") == 0) {
				...
				close();
				P_ENDLESS_CTRL->getStrengthenPanel()->open();
			}
		break;
		default:
