...
CFightEndlessShowPanel::CFightEndlessShowPanel(){
	m_pBaoxiangAni = NULL;
	m_pCurCellAni = NULL;
}
...
bool CFightEndlessShowPanel::init() {
	bool bRet = false;
	...
	do {
		...
		m_pDoOKBtn->addTouchEventListener(this, toucheventselector(CFightEndlessShowPanel::touchEventHandler));
...
void CFightEndlessShowPanel::open(){
	BasePanel::open();
...
void CFightEndlessShowPanel::close(){
	BasePanel::close();
...
void CFightEndlessShowPanel::touchEventHandler(CCObject *pSender, TouchEventType type) {
	...
	case TOUCH_EVENT_ENDED:
		if(std::strcmp(pszWidgetName, "sure_btn") == 0) {
			close();
			P_ENDLESS_CTRL->getResultPanel()->open();
			P_ENDLESS_CTRL->getResultPanel()->updateUI();
		} 
	default:
		break;
	}
...
void CFightEndlessShowPanel::showResult(const CSInfiniteEndRsp& stRsp){
	if(!isOpen()) {
		return;
	}
	...
	if(m_pBaoxiangAni) {
		...
		m_pBaoxiangAni->getAnimation()->setMovementEventCallFunc(this, movementEvent_selector(CFightEndlessShowPanel::HandleBaoxiangEffEvent));
...
void CFightEndlessShowPanel::HandleBaoxiangEffEvent(CCArmature * armature, MovementEventType type, const char * name) {
	if (type == COMPLETE){
		showNextElmt();
		if(m_pBaoxiangAni) {
...
void CFightEndlessShowPanel::HandleCellEffEvent(CCArmature * armature, MovementEventType type, const char * name) {
	if (type == COMPLETE){
		if(m_pCurCellAni) {
...
void CFightEndlessShowPanel::showNextElmt() {
	...
	if(m_pCurCellAni) {
		CCPoint stPt = pstCell->getCell()->getPosition(); 
		m_pCurCellAni->setPosition(stPt.x, stPt.y);
		m_pCurCellAni->getAnimation()->setMovementEventCallFunc(this, movementEvent_selector(CFightEndlessShowPanel::HandleCellEffEvent));
...
const string CFightEndlessShowPanel::getLastCellAniName(const bool isLoopAni) {
...
const std::string CFightEndlessShowPanel::getColorString(const int iColor) {
	switch(iColor) {
	case 1:
		return "white";
	case 2:
